...
def ctx_home(tmp_path, monkeypatch):
    ...
    # ctx.store re-exports ctx_capture.store's _ctx_dir rather than
    # defining its own, so patching the one canonical home is sufficient.
...
def populated_db(ctx_home):
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        conn.execute(
            "INSERT INTO runs VALUES (1, 1, ?, 'pipe_a', '2026-06-08T10:05:00+00:00', ?)",
            (rec_minimal.query, json.dumps(rec_minimal.to_json())),
        )
        conn.execute(
            "INSERT INTO runs VALUES (2, 1, ?, 'pipe_a', '2026-06-09T10:05:00+00:00', ?)",
            (rec_full.query, json.dumps(rec_full.to_json())),
        )
        conn.execute(
            "INSERT INTO runs VALUES (2, 2, ?, 'pipe_a', '2026-06-09T10:10:00+00:00', ?)",
            (rec_other.query, json.dumps(rec_other.to_json())),
        )
